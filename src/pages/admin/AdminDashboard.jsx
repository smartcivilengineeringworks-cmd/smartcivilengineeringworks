...
const AdminDashboard = () => {
  ...
  const handleDeleteInquiry = async (id) => {
    if (!window.confirm('Are you sure you want to permanently delete this client inquiry?')) return;
    try {
  ...
  const handleExportInquiriesCSV = () => {
    ...
    const headers = ['ID', 'Date', 'Status', 'Name', 'Email', 'Phone', 'Subject', 'Message'];
    ...
    link.href = url;
    link.setAttribute('download', `smart-civil-leads-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans text-slate-700 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full space-y-6">
        ...
        {activeTab === 'inquiries' && (
          <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden p-6 space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium mt-0.5">
                  Manage incoming client project requests, quotation inquiries, and visitor leads.
                </p>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-navy hover:text-white text-navy text-xs font-bold uppercase tracking-wider transition-all flex items-center space-x-1.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <div className="flex items-center space-x-2">
              {[
                ...
                { id: 'Unread', label: `Unread (${inquiries.filter(i => !i.is_read).length})` },
                { id: 'Read', label: `Read (${inquiries.filter(i => i.is_read).length})` }
              ].map((tab) => (
                <button
                  key={tab.id}
            ...
            {inquiries
              ...
              <div className="text-center py-12 text-slate-400 text-xs font-medium">
                {inquiries.length === 0
                  ...
                  : `No ${inquiryFilter.toLowerCase()} inquiries found.`}
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="pt-3 flex items-center justify-between border-t border-slate-100 mt-2">
                        ...
                        <button
                          ...
                          className="flex items-center space-x-1 text-xs text-slate-400 hover:text-rose-600 transition-colors cursor-pointer px-2 py-1 rounded-md hover:bg-rose-50"
              ...
              <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-200/60">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-[10px] text-slate-400">Upload photo directly from your device</span>
                  </div>
                </div>

                {/* Upload Button Box */}
                <label className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-300 hover:border-accent rounded-xl bg-white cursor-pointer transition-colors group">
                  <Upload className="h-7 w-7 text-slate-400 group-hover:text-accent mb-2 transition-colors" />
                  <span className="text-xs font-bold text-slate-700 group-hover:text-navy">
                    {imageLoading ? 'Optimizing photo for web...' : 'Click or tap to upload photo from your device'}
                  </span>
                  <span className="text-[10px] text-slate-400 mt-1">
                    {imageLoading ? 'Resizing and compressing automatically...' : 'Supports PNG, JPG, JPEG, WEBP (automatically optimized for fast loading)'}
                  </span>
                  <input
                    type="file"
                    accept="image/*"
                ...
                {projectForm.image_url && (
                  <div className="flex items-center space-x-3 p-2.5 bg-white rounded-xl border border-slate-200 mt-2">
                    <div className="h-16 w-24 rounded-lg overflow-hidden bg-slate-100 shrink-0 border border-slate-200">
                      <img
                    ...
                    <div className="min-w-0 flex-1">
                      <span className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider flex items-center space-x-1">
                        ...
                        <span>Selected Image Ready</span>
                      </span>
                      <p className="text-[11px] text-slate-500 truncate mt-0.5">
                        Photo ready to be published
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setProjectForm({ ...projectForm, image_url: '' })}
                      className="text-[10px] text-rose-500 hover:text-rose-700 font-bold px-2.5 py-1.5 cursor-pointer hover:bg-rose-50 rounded-lg transition-colors"
                    >
                      Remove
                    </button>
                  </div>
